...
export (bool) var merge_mesh_children = false setget execute_merge


func execute_merge(value):
	
	var merged_mesh = merge_meshinstances(get_children(), Spatial.new(), false, false)
	get_tree().get_edited_scene_root().add_child(merged_mesh)
	merged_mesh.set_owner(get_tree().get_edited_scene_root())
...
func merge_meshinstances(var mesh_array, var attachment_node : Node, var use_local_space : bool = false, var delete_originals : bool = true):
	...
	#new_mi.set_owner(attachment_node)
